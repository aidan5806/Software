...
def getinfo():                      #Gets values from the gamepad every change of state
    ...
    for event in events:            #Operates on the most recent event, will not skip events
        ...
        elif stick =='BTN_NORTH' and stickval == 1:
            message = '@'
            s.sendall(message.encode('ascii'))
        elif stick =='BTN_SOUTH' and stickval == 1:
            message = '~'
            s.sendall(message.encode('ascii'))
...
while True:
    ...
    if (int(time.clock()*10))%1==0:                     #Execute five times a second
        ...
        print(message)                                  #Also prints message for
        time.sleep(0.1)                                 #waits for 200 milliseconds
#------------------------------------------------------------------------------------------#
